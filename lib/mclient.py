...
class Lock(object):
    """
    with type("_lock", (object,) dict(client=MClient(servers,True)))('key',):
        do ...
    """
    
    def __init__(self, key, retry_count=20, interval=0.2, expire=5, value=None):
        """
        :param str key: 锁定key
        :param int retry_count: 重试次数
        :param float interval: 重试间隔
        :param int expire: key过期时间
        :param object value: key对应的数值
        ...
        self.lock_value = value if value else key
        self.lock_retry_count = retry_count
        self.lock_interval = interval
        
    def __enter__(self):
        # 锁定当前的key
        locked = False
        for x in xrange(self.lock_retry_count):
            if not self.client.mutex(self.lock_key, self.lock_expire, self.lock_value):
                time.sleep(self.lock_interval)
                logging.debug('lock key failure: %s', self.lock_key)
            else:
                locked = True
                logging.debug('lock key success: %s', self.lock_key)
                break
            
            pass

        if not locked:
            raise RuntimeError('MClient.Lock expire: key: %s, expire: %s' % (self.lock_key, self.lock_expire))

    def __exit__(self, _type, _error, _traceback):
        # 解锁当前的key
        self.client.unmutex(self.lock_key)
        logging.debug('unlock key: %s', self.lock_key)

        if _type is None and _error is None and _traceback is None:
            return True
        else:
            return False
