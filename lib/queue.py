...
import hashlib
import amqp
from lib.utils import json_default
...
class RedisQueue(object):
    ...
    def __init__(self, redis, queue_key, queue_num):
        """
        :param redis: redis实例
        :param queue_key: 队列的key
        :param queue_num: 队列的个数
        """
        self.redis = redis
    ...
    def push_queue(self, partition_value, msg_data):
        """
        消息入队
        :param partition_value: 队列hash的值
        :param msg_data: 消息
        :return: 是否成功入队 True or False
        """
        queue_id = int(hashlib.md5(partition_value).hexdigest()[0:2], 16) % self.queue_num
        if self.redis.rpush(self.queue_key % queue_id, json.dumps(msg_data, default=json_default)):
            return True
        return False
