...
class TimeInterval(object):
    ...
    def _isValidTime(self, hour, minute):
        return isinstance(hour, int) and isinstance(minute, int) and hour >= 0 and \
               hour <=23 and minute >=0 and minute <= 59
    
    def _isValidInterval(self):
        return self.endHour * 60 + self.endMin >= self.startHour * 60 + self.startMin
        
    def GetObject(self):
        if not (self._isValidTime(self.startHour, self.startMin) and \
           self._isValidTime(self.endHour, self.endMin)):
            return None
        ...
        return {
                self.STR_START:{self.STR_HOUR:str(self.startHour),
...
class ClickAction(object):
    ...
    def GetObject(self):
        ...
        if self.TYPE_ACTIVITY == self.actionType:
            ...
            ret['package_name'] = {'packageDownloadUrl':self.packageDownloadUrl,
                                   'confirm':self.confirmOnPackage,
...
class XingeHelper(object):
    ...
    def Request(cls, path, params):
        ...
        if 200 != response.status:
            ...
            logging.warn('push response %s', retDict)
